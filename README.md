...
当前推荐从邀请码模式升级为“管理员分配账号”：

1. 管理员使用登录名和密码进入系统。
2. 后台创建人员登录名、初始密码、角色、可用天数。
3. 用户每天登录后会显示剩余可用天数。
4. 过期账号无法登录。
5. 学生只能进入学习区，不能使用 AI 问诊和门诊业务。
6. 医生可以使用 AI 推理、门诊挂号、病例归档。
7. 管理员可以进入后台，管理账号、学习资料、诊断词条和方药知识。
...
主页顶部普通用户默认看不到后台和 GitHub。只有管理员登录后，主页右上角才显示后台入口。后台负责账号、资料、诊断词条、方药知识和数据库管理。
...
文件底部有初始化管理员账号的 SQL，请把 `admin` 和 `CHANGE_ADMIN_PASSWORD` 改成自己的管理员登录名和密码。

学习区支持 `learning_resources` 表，后续可接 Supabase Storage 上传 PDF、TXT、图片、音频、视频、Office/WPS 等文件。大文件不建议直接塞进数据库，应上传到 Storage，数据库只保存标题、类型、说明和文件路径。

可优先调研/尝试的中医 AI 模型或项目方向：

- BianCang / 扁仓：中医大语言模型方向
- Qibo / 岐伯：中医问答和诊疗语料方向
- JingFang：经方相关模型/数据方向
- ZhiFangDanTai：GraphRAG 方剂生成研究方向

这些模型即使可用，也只能作为研究和教学辅助，不能替代执业医师诊疗。
